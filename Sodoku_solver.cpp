...
void sudoku::write_notation(const uint8_t cell, const uint8_t number, const uint8_t state)
{
    if (state == u8sc(0))
    {
        board[cell] &= (u16sc(~(u8sc(1) << (u8sc(3) + number))));
    }
    else if (state == u8sc(1))
    {
        board[cell] |= (u16sc(u8sc(1) << (u8sc(3) + number)));
    }
}
...
{
    const uint8_t number = u8sc((*this)[cell]);
    const uint8_t col = cell % u8sc(9);
    const uint8_t row = cell / u8sc(9);
    // iterate through the column
    for (uint8_t i = col; i < u8sc(81); i += u8sc(9))
    {
        write_notation(i, number, 0);
    }
    // iterate through the row
    for (uint8_t i = 0; i < u8sc(9); i++)
    {
        write_notation(row * u8sc(9) + i, number, 0);
    }
    // iterate through the square
    const uint8_t s_col = (cell % u8sc(9)) / u8sc(3);
    const uint8_t s_row = (cell / u8sc(9)) / u8sc(3);
    for (uint8_t i = 0; i < u8sc(3); i++)
    {
        for (uint8_t j = 0; j < u8sc(3); j++)
        {
            const uint8_t index = (s_row * u8sc(3) + i) *
                u8sc(9) + (s_col * u8sc(3) + j);
...
bool sudoku::get_notation(const uint8_t cell, const uint8_t number) const
{
    ...
    return board[cell] >> (number + u8sc(3)) & 1;
}
...
bool sudoku::solve_by_sudoku()
{
    ...
    for (uint8_t i = 0; i < u8sc(81); i++)
    {
        ...
        if ((*this)[i] == u8sc(0))
        {
            bool found = solve_by_column(i % u8sc(9), i);
            if (found)
            {
                return found;
            }
            found = solve_by_row(i / u8sc(9), i);
            if (found)
            {
                return found;
            }
...
bool sudoku::solve_by_row(const uint8_t row, const uint8_t cell)
{
    ...
    const uint8_t index = row * u8sc(9);
    // iterate through row
    for (uint8_t i = index; i < index + u8sc(9); i++)
    {
        // skip empty cells
        if ((*this)[i] == u8sc(0))
        {
        ...
        if (temp.size() == u8sc(8))
        {
            for (uint8_t number = 1; number <= u8sc(9); number++)
            {
...
bool sudoku::solve_by_square(const uint8_t col, const uint8_t row, const uint8_t cell)
{
    ...
    for (uint8_t i = 0; i < u8sc(3); i++)
    {
        ...
        for (uint8_t j = 0; j < u8sc(3); j++)
        {
            ...
            const uint8_t index = 
                (row * u8sc(3) + i) * u8sc(9) + (col * u8sc(3) + j);
            // skip empty cells
            if ((*this)[index] == u8sc(0))
            {
                continue;
            }
...
{
    std::vector possibilities(u8sc(81), std::vector<uint8_t>(9, 1));
    // iterate through all columns
    for (uint8_t col = 0; col < u8sc(9); col++)
    {
        for (uint8_t i = col; i < u8sc(81); i += u8sc(9))
        {
            ...
            if ((*this)[i] != u8sc(0))
            {
                for (uint8_t j = col; j < u8sc(81); j += u8sc(9))
                {
                    possibilities[j][(*this)[i] - 1] = u8sc(0);
                }
            }
        }
    }
    // iterate through all rows
    for (uint8_t row = 0; row < u8sc(9); row++)
    {
        for (uint8_t i = 0; i < u8sc(9); i++)
        {
            // if we encounter a number in the row remove it from the list of possible numbers
            if ((*this)[row * u8sc(9) + i] != u8sc(0))
            {
                for (uint8_t j = 0; j < u8sc(9); j++)
                {
                    possibilities[row * u8sc(9) + j]
                    [(*this)[row * u8sc(9) + i] - 1] = u8sc(0);
                }
            }
        }
    }
    // iterate through all squares
    for (uint8_t cell = 0; cell < u8sc(81); cell++)
    {
        // calculate the current row and column of the square
        // top left square is 0, 0, top middle square is 1, 0, top right square is 2, 0
        // middle left square is 0, 1, middle square is 1, 1, middle right square is 2, 1
        // bottom left square is 0, 2, bottom middle square is 1, 2, bottom right square is 2, 2
        const uint16_t s_col = (cell % u8sc(9)) / u8sc(3);
        const uint16_t s_row = (cell / u8sc(9)) / u8sc(3);
        ...
        for (uint8_t i = 0; i < u8sc(3); i++)
        {
            ...
            {
                // index is the cell in the square we are checking
                const uint16_t index = (s_row * u8sc(3) + i) * u8sc(9) + 
                    (s_col * u8sc(3) + j);
                // if we encounter a number in the square remove it from the list of possible numbers
                if ((*this)[index] != u8sc(0))
                {
                    // iterate through rows of the square
                    for (uint8_t k = 0; k < u8sc(3); k++)
                    {
                        // iterate within row
                        for (uint8_t l = 0; l < u8sc(3); l++)
                        {
                            const uint16_t write_index = (s_row * u8sc(3) + k) * 
                                u8sc(9) + (s_col * u8sc(3) + l);
                            possibilities[write_index][(*this)[index] - 1] = u8sc(0);
                        }
                    }
                }
            }
        ...
        for (uint8_t i = 0; i < u8sc(9); i++)
        {
            if (possibilities[cell][i] != u8sc(0) && (*this)[cell] == u8sc(0))
            {
...
{
    for (int i = u8sc(0); i < u8sc(81); i++)
    {
        if ((board[i] & MASK) == u8sc(0))
        {
            board[i] = u8sc(0);
        }
    }
...
bool sudoku::solve_by_single_candidate()
{
    ...
    for (uint8_t number = 1; number <= u8sc(9); number++)
    {
        ...
        for (uint8_t row = 0; row < u8sc(9); row++)
        {
            ...
            for (uint8_t i = 0; i < u8sc(9); i++)
            {
                const uint8_t cell = row * u8sc(9) + i;
        ...
        // check all squares
        // iterate through all squares
        for (uint8_t cell = 0; cell < u8sc(81); cell += u8sc(7))
        {
            // calculate the current row and column
            const uint8_t col = (cell % u8sc(9)) / u8sc(3);
            const uint8_t row = (cell / u8sc(9)) / u8sc(3);
            // iterate through rows of the square
            uint8_t counter = 0;
            uint8_t found = 0;
            bool skip = false;
            for (uint8_t i = 0; i < u8sc(3); i++)
            {
                ...
                {
                    // index is the cell in the square we are checking
                    const uint8_t index = (row * u8sc(3) + i) *
                        u8sc(9) + (col * u8sc(3) + j);
    ...
    for (uint8_t cell = 0; cell < u8sc(81); cell++)
    {
        ...
        if (count == u8sc(2))
        {
            ...
            for (uint8_t i = col; i < u8sc(81); i += u8sc(9))
            {
                ...
                if (count == u8sc(2) && pair1 == pair2)
                {
                    for (uint8_t k = col; k < u8sc(81); k += u8sc(9))
                    {
                        if (k == cell || k == i)
                        {
                            continue;
            ...
            if (found == false)
            {
                ...
                for (uint8_t i = 0; i < u8sc(9); i++)
                {
                    ...
                    count = u8sc(0);
                    std::vector<uint8_t> pair2{};
                    const uint8_t index = row * u8sc(9) + i;
                    // ignore the cell we already found
                    if (index == cell)
                    {
                        continue;
                    ...
                    if (count == u8sc(2) && pair1 == pair2)
                    {
                        for (uint8_t k = 0; k < u8sc(9); k++)
                        {
                            const uint8_t temp = row * u8sc(9) + k;
                ...
                for (uint8_t j = 0; j < u8sc(3); j++)
                {
                    ...
                    if (count == u8sc(2) && pair1 == pair2)
                    {
                        ...
                        for (uint8_t k = 0; k < u8sc(3); k++)
                        {
                            ...
                            for (uint8_t l = 0; l < u8sc(3); l++)
                            {
                                const uint8_t final_index = (s_row * u8sc(3) + k) * 
                                    u8sc(9) + (s_col * u8sc(3) + l);
            ...
            for (uint8_t i = col; i < u8sc(81); i += u8sc(9))
            {
                    ...
                    && tiple_or_quad(triple1, triple2) == true)
                {
                    ...
                    if (cell2 != u8sc(100) && cell2 != i)
                    {
                        ...
                        std::cout << "Found a naked triple at " << u16sc(cell) << " " 
                            << u16sc(cell2) << " " << u16sc(i) << '\n';
                    }
                }
            }
